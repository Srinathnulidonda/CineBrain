class FilterManager {
    ...
    setupAdvancedFilters() {
        ...
        const savedContentSearch = document.getElementById('savedContentSearch');

        if (recommendationsFilter) {
            recommendationsFilter.addEventListener('change', (e) => {
                this.handleRecommendationsFilter(e.target.value);
            });
        }

        if (recommendationsSort) {
            recommendationsSort.addEventListener('change', (e) => {
                this.handleRecommendationsSort(e.target.value);
            });
        }

        if (savedContentFilter) {
            savedContentFilter.addEventListener('change', (e) => {
                this.handleSavedContentFilter(e.target.value);
            });
        }

        if (savedContentSearch) {
            savedContentSearch.addEventListener('input', (e) => {
                this.handleSavedContentSearch(e.target.value);
            });

            // Clear search functionality
            savedContentSearch.addEventListener('keydown', (e) => {
                if (e.key === 'Escape') {
                    e.target.value = '';
                    this.handleSavedContentSearch('');
                }
            });
        }

        // Add clear search button for saved content
        this.addClearSearchButton();
    }

    handleRecommendationsFilter(filterValue) {
        console.log('🔍 Recommendations filter changed:', filterValue);

        this.manager.state.filters.recommendations = filterValue;
        this.saveFilterState();
        this.showFilterLoading('recommendations');

        // Clear cache for this filter combination
        this.clearFilterCache('recommendations');

        // Reload with new filter
        this.manager.loadRecommendations(true).finally(() => {
            this.hideFilterLoading('recommendations');
        });

        // Update URL if needed
        this.updateURL('rec_filter', filterValue);
    }

    handleRecommendationsSort(sortValue) {
        console.log('📊 Recommendations sort changed:', sortValue);

        this.manager.state.sorting.recommendations = sortValue;
        this.saveFilterState();
        this.showFilterLoading('recommendations');

        // Clear cache for this sort combination
        this.clearFilterCache('recommendations');
        ...
        // Update URL if needed  
        this.updateURL('rec_sort', sortValue);
    }

    handleSavedContentFilter(filterValue) {
        console.log('🔍 Saved content filter changed:', filterValue);

        this.manager.state.filters.upcoming = filterValue;
        this.saveFilterState();
        this.showFilterLoading('saved-content');

        // Clear cache
        this.clearFilterCache('upcoming');

        // Reload with new filter
        this.manager.loadUpcomingRecommendations(true).finally(() => {
            this.hideFilterLoading('saved-content');
        });

        // Update URL
        this.updateURL('saved_filter', filterValue);
    }

    handleSavedContentSearch(searchValue) {
        // Debounce search input
        this.debounce('savedContentSearch', () => {
            console.log('🔎 Saved content search:', searchValue);

            this.manager.state.savedContentSearchQuery = searchValue.trim();
            this.saveFilterState();
    ...
    showFilterLoading(section) {
        const container = this.getFilterContainer(section);
        if (container) {
            container.classList.add('loading-filters');

            // Add loading indicator
            const loadingIndicator = container.querySelector('.filter-loading-indicator');
            if (!loadingIndicator) {
                const indicator = document.createElement('div');
                indicator.className = 'filter-loading-indicator';
                indicator.innerHTML = '<i data-feather="rotate-cw" class="spinning"></i>';
    ...
    hideFilterLoading(section) {
        const container = this.getFilterContainer(section);
        if (container) {
            container.classList.remove('loading-filters');
    ...
    getFilterContainer(section) {
        switch (section) {
            case 'recommendations':
                return document.querySelector('.recommendations-controls');
            case 'saved-content':
                return document.querySelector('.saved-content-controls');
            default:
                return null;
        }
    }

    clearFilterCache(type) {
        const cacheKeys = Array.from(this.filterCache.keys());
        ...
        // Also clear manager cache
        if (this.manager.cache) {
            this.manager.cache.lastUpdated[type] = 0;
        }
    }
    ...
    saveFilterState() {
        try {
            const filterState = {
                recommendations: {
                    filter: this.manager.state.filters.recommendations,
                    sort: this.manager.state.sorting.recommendations
                },
                savedContent: {
                    filter: this.manager.state.filters.upcoming,
                    search: this.manager.state.savedContentSearchQuery || ''
                },
                timestamp: Date.now()
            };

            localStorage.setItem('cinebrain-admin-filters', JSON.stringify(filterState));
        } catch (error) {
    ...
    loadFilterState() {
        try {
            const saved = localStorage.getItem('cinebrain-admin-filters');
            if (saved) {
    ...
    restoreFilters(filterState) {
        // Restore recommendations filters
        if (filterState.recommendations) {
            const recFilter = document.getElementById('recommendationsFilter');
            const recSort = document.getElementById('recommendationsSort');

            if (recFilter && filterState.recommendations.filter) {
                recFilter.value = filterState.recommendations.filter;
                this.manager.state.filters.recommendations = filterState.recommendations.filter;
            }

            if (recSort && filterState.recommendations.sort) {
                recSort.value = filterState.recommendations.sort;
                this.manager.state.sorting.recommendations = filterState.recommendations.sort;
            }
        }

        // Restore saved content filters
        if (filterState.savedContent) {
            const savedFilter = document.getElementById('savedContentFilter');
            const savedSearch = document.getElementById('savedContentSearch');

            if (savedFilter && filterState.savedContent.filter) {
                savedFilter.value = filterState.savedContent.filter;
                this.manager.state.filters.upcoming = filterState.savedContent.filter;
            }

            if (savedSearch && filterState.savedContent.search) {
                savedSearch.value = filterState.savedContent.search;
                this.manager.state.savedContentSearchQuery = filterState.savedContent.search;
            }
        }
    }
    ...
    addClearSearchButton() {
        const savedSearchInput = document.getElementById('savedContentSearch');
        if (savedSearchInput && !savedSearchInput.nextElementSibling?.classList.contains('search-clear-saved')) {
            const clearBtn = document.createElement('button');
            clearBtn.type = 'button';
            clearBtn.className = 'search-clear-saved';
            clearBtn.innerHTML = '<i data-feather="x"></i>';
            ...
            savedSearchInput.addEventListener('input', () => {
                clearBtn.style.display = savedSearchInput.value ? 'flex' : 'none';
            });

            savedSearchInput.parentNode.style.position = 'relative';
            savedSearchInput.parentNode.appendChild(clearBtn);
            window.recUtils.refreshFeatherIcons();
        }
    }

    resetAllFilters() {
        // Reset recommendations filters
        const recFilter = document.getElementById('recommendationsFilter');
        const recSort = document.getElementById('recommendationsSort');

        if (recFilter) recFilter.value = 'all';
        if (recSort) recSort.value = 'created_at';

        // Reset saved content filters  
        const savedFilter = document.getElementById('savedContentFilter');
        const savedSearch = document.getElementById('savedContentSearch');

        if (savedFilter) savedFilter.value = 'all';
        if (savedSearch) savedSearch.value = '';

        // Reset state
        this.manager.state.filters.recommendations = 'all';
        this.manager.state.filters.upcoming = 'all';
        this.manager.state.sorting.recommendations = 'created_at';
        this.manager.state.savedContentSearchQuery = '';
        ...
        // Clear saved state
        localStorage.removeItem('cinebrain-admin-filters');

        this.manager.showToast('Filters reset successfully', 'success');
    }
}
...
class AdminRecommendations {
    ...
    async loadRecommendations(forceRefresh = false) {
        ...
        try {
            ...
            const filterBy = this.state.filters.recommendations || 'all';
            ...
            if (filterBy !== 'all') {
                if (['featured', 'trending', 'hidden_gem', 'classic', 'new_release', 'mind_bending', 'anime_gem', 'scene_clip', 'top_list'].includes(filterBy)) {
                    params.append('recommendation_type', filterBy);
            ...
            if (response.ok) {
                ...
                if (filterBy === 'draft') {
                    recommendations = recommendations.filter(rec => !rec.is_active);
                }
                ...
                // Sort recommendations if needed
                recommendations = this.sortRecommendations(recommendations, sortBy);
    ...
    async loadUpcomingRecommendations(forceRefresh = false) {
        ...
        try {
            ...
            // Add type filter
            const typeFilter = this.state.filters.upcoming;
            if (typeFilter && typeFilter !== 'all') {
                params.append('recommendation_type', typeFilter);
            }

            // Add search parameter
            const searchQuery = this.state.savedContentSearchQuery;
            ...
            if (response.ok) {
                ...
                // Client-side search if server doesn't support it
                if (searchQuery && searchQuery.trim().length >= 2) {
            ...
            this.showError('Failed to load upcoming recommendations: ' + error.message);
        }
    }
    ...
    sortRecommendations(recommendations, sortBy) {
        switch (sortBy) {
            case 'created_at':
                return recommendations.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
            case 'updated_at':
                return recommendations.sort((a, b) => new Date(b.updated_at) - new Date(a.updated_at));
            case 'type':
                return recommendations.sort((a, b) => (a.recommendation_type || '').localeCompare(b.recommendation_type || ''));
            case 'title':
                return recommendations.sort((a, b) => (a.content?.title || '').localeCompare(b.content?.title || ''));
            default:
